...
class QMIX_ECE_v2(QMIX):
    ...
    def learn(self, total_steps):
        ...
        while step < total_steps:
            if step >= 0.2*total_steps:
                if not switch_flag:
                    generation = generation*n_populations
                switch_flag = True
                with torch.no_grad():
                    total_reward, step = self.runner.run(step)
                ...
                with torch.no_grad():
                    ...
                    for individual in populations:
                        fitness, episode_steps, mean_td_error, total_reward, epsilon_restriction = self.evaluate(individual.actions)
                        # if step >= 0.2*total_steps:
                        #     fitness -= (1-self.runner.epsilon)*total_reward
                        #     self.ga_config['crossover_rate'] = self.runner.epsilon - 0.2
                        # else:
                        fitness -= 5*(1-self.runner.epsilon)*epsilon_restriction
                        ...
                        if n_eval < n_populations:
                            infos.append(info)
                        n_eval+=1
                step += episode_steps*n_populations
                #self._debug(populations, 'evaluation')
                ...
                #self._debug(populations, 'survivor')
                parents_idx = SGA.parent_select(populations, k=5)
                #self._debug(populations, 'parent select')
                offsprings = SGA.crossover(populations, parents_idx, self.ga_config['crossover_rate'])
                #self._debug(populations, '(parent)crossover')
                #self._debug(offsprings, '(offspring)crossover')
                SGA.mutation(offsprings, self.ga_config['mutation_rate'], low=-1, high=self.n_actions)
                #self._debug(populations, '(parent)mutation')
                #self._debug(offsprings, '(offspring)mutation')
                populations = populations + offsprings # (mu+lambda)
                #self._debug(populations, 'after')
                self.runner.update_epsilon(step)

            generation += 1
            if switch_flag:
                if generation % 1000 == 0:
                    print('Steps: %d\tEpsilon:%.2f\tEp.Reward: %.2f' % (step, self.runner.epsilon, total_reward))
            else:
                print('Steps: %d\tEpsilon:%.2f\tEp.Reward: %.2f\tFitness: %.2f\tTD-Error: %.2f\tEpsilon.Restrct %.2f' % (step, self.runner.epsilon, total_reward, info['Fitness'], info['TD-Error'], epsilon_restriction))
    ...
    def evaluate(self, all_actions):
        ...
        epsilon = self.runner.epsilon
        #fitness = .25*epsilon*mean_td_error*step + (1-epsilon)*total_reward
        epsilon_restriction = self.epislon_restriction(all_actions)
        fitness = 0.25*epsilon*mean_td_error*step + (1-epsilon)*total_reward + 5*(1-epsilon)*epsilon_restriction
        return fitness, step, mean_td_error, total_reward, -epsilon_restriction

    def epislon_restriction(self, actions:np.ndarray):
        value = (actions == -1).sum()/len(actions)*100
        return -(value-(1-self.runner.epsilon)*100)**2/10
        
    def _debug(self, populations, loc=''):
        print(loc)
